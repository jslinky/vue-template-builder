<template>
  ...
  <div :class="item.classes.applied">
    ...
    <div v-if="edit">
      <div v-for="(value, key) in item.classes" class="o-buttons">
        <a class="o-btn" v-if="omitApplied(value)" @click="addClass(value)">{{ key }}</a>
      </div>
    </div>  
    </section>
</template>
...
export default {
  ...
  methods: {
    ...
    addClass(value) {
      let classesApplied = this.item.classes.applied; 
